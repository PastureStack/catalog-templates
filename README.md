...
when a later version is added. This repository therefore retains version `1` of
Network Diagnostics, Network Policy Manager, and Secret Volume alongside their
current version `2` definitions. The retained Compose definitions are restored
from the reviewed `v0.3.0-rc21` tag; Taiwan Traditional Chinese readmes are
provided without changing the deployed workload definition. The integration
gate resolves both retained and current version IDs through Catalog Service so
an existing stack cannot regress to a version-detail 404.
