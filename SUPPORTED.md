...
| PastureStack Kubernetes Cluster | Infrastructure orchestration stack | v1.12.10-pasturestack.1 | Nine reviewed public PastureStack GHCR images with semantic version tags | Source, license, vulnerability, anonymous-pull, template-rendering, and isolated image gates passed; full two-host stack lifecycle pending |
...
Catalog version `1` remains available for already-installed stacks and retains
the reviewed `v0.2.0` agent and service image tags. Version `2` is the current
default and adds localized question metadata without changing those image tags.
...
## PastureStack Network Policy Manager evidence

Catalog version `1` remains available for already-installed stacks and retains
the reviewed `v0.3.1` image tag. Version `2` is the current default metadata
revision.

- Image: `ghcr.io/pasturestack/network-policy-manager:v0.3.1`
- Source: [`PastureStack/network-policy-manager@dfcfc9e22126208587176130d9e6279245d502ba`](https://github.com/PastureStack/network-policy-manager/tree/dfcfc9e22126208587176130d9e6279245d502ba)
- Release: [`v0.3.1`](https://github.com/PastureStack/network-policy-manager/releases/tag/v0.3.1)
- Upstream: [`rancher/network-policy-manager`](https://github.com/rancher/network-policy-manager)
- License: Apache-2.0 for the project; Ubuntu, nftables, and bundled dependencies retain their upstream licenses and notices
- Reviewed: 2026-07-25
- Vulnerability gate: 0 HIGH and 0 CRITICAL findings with Trivy 0.70.0
- Runtime gate: five consecutive two-host runs passed baseline connectivity, default deny, directed TCP/8080 allow, rollback, temporary-credential removal, and temporary-stack removal
- Stability gate: both agents remained ready with zero failures and zero restarts after the repeated lifecycle

The agent coalesces established sidekick service aliases that share a service
identifier while rejecting conflicting workload views. It owns only
`table inet pasturestack_policy`, updates it atomically, preserves
last-known-good rules during bounded Metadata failures, and removes only that
table during a graceful Catalog removal. It uses host networking and
`NET_ADMIN`, drops every other capability, and does not require privileged
mode, host PID, a container-engine socket, host filesystem mounts, API
credentials, or secret input.
...
## PastureStack Secret Volume evidence

Catalog version `1` remains available for already-installed stacks and retains
the reviewed `v0.1.1` image tag. Version `2` is the current default metadata
revision.

- Image: `ghcr.io/pasturestack/secrets-flexvolume-plugin:v0.1.1`
- Source: [`PastureStack/secrets-flexvolume-plugin@6f0d6ed72a37ad9b8169a355e9bcb99472acb7fe`](https://github.com/PastureStack/secrets-flexvolume-plugin/tree/6f0d6ed72a37ad9b8169a355e9bcb99472acb7fe)
- Release: [`v0.1.1`](https://github.com/PastureStack/secrets-flexvolume-plugin/releases/tag/v0.1.1)
- Upstream: [`rancher/secrets-flexvol`](https://github.com/rancher/secrets-flexvol)
- License: Apache-2.0 for the project; Ubuntu, the Go toolchain, CA
  certificates, and bundled dependencies retain their upstream licenses and
  notices
- Reviewed: 2026-07-25
- Vulnerability gate: 0 HIGH, 0 CRITICAL, and 0 detected image secrets with
  Trivy 0.70.0
- Runtime gates: signed-envelope verification, RSA-OAEP-SHA256 unwrap,
  HMAC-SHA256 authentication, AES-256-GCM decryption, read-only UID/GID modes,
  multiple consumers, final-unmount cleanup, readiness, anonymous pull, and
  restricted-container startup

The driver exposes Docker Volume Plugin API v1 under the
`pasturestack-secret-volume` name. It retrieves only the encrypted records
authorized by the opaque volume token and materializes them in a bounded
`tmpfs` with `nodev`, `nosuid`, and `noexec`. The service drops all
capabilities before adding only `SYS_ADMIN` for the memory-backed mount. It
does not request privileged mode, host networking, a host PID namespace, a
container-engine socket, or unrestricted host storage. The Catalog health
strategy reports faults without automatically recreating the container and
disturbing active mounts. A complete two-host, Catalog-created secret workload
lifecycle remains required before production approval.

## PastureStack Vault Volume evidence

- Bridge image:
  `ghcr.io/pasturestack/vault-secrets-bridge:v0.1.1`
- Bridge source:
  [`PastureStack/vault-secrets-bridge@fd63a1153aa7556ba21f8099785508ff3e693c4d`](https://github.com/PastureStack/vault-secrets-bridge/tree/fd63a1153aa7556ba21f8099785508ff3e693c4d)
- Bridge release:
  [`v0.1.1`](https://github.com/PastureStack/vault-secrets-bridge/releases/tag/v0.1.1)
- Driver image:
  `ghcr.io/pasturestack/secrets-flexvolume-plugin:v0.2.0`
- Driver source:
  [`PastureStack/secrets-flexvolume-plugin@65bccd483d06968af445f4075a65ceff5fe26806`](https://github.com/PastureStack/secrets-flexvolume-plugin/tree/65bccd483d06968af445f4075a65ceff5fe26806)
- Driver release:
  [`v0.2.0`](https://github.com/PastureStack/secrets-flexvolume-plugin/releases/tag/v0.2.0)
- License: Apache-2.0 for both projects; Ubuntu, the Go toolchain, CA
  certificates, and bundled dependencies retain their upstream licenses and
  notices
- Reviewed: 2026-07-25
- Vulnerability gate: both images have 0 HIGH, 0 CRITICAL, and 0 detected image
  secrets with Trivy 0.70.0
- Runtime gates: real Vault response wrapping and unwrap, RSA-PSS host
  authentication, policy allowlisting, replay rejection, `0400` non-root Secret
  input, read-only token materialization, driver restart recovery, final
  accessor revocation, anonymous pulls, and exact cleanup

The issuing token is selected from an existing platform Secret and mounted
read-only into the unprivileged bridge. It is never inserted into Compose
environment values or command-line arguments. The global driver drops all
capabilities before adding only `SYS_ADMIN`, `CHOWN`, and `FOWNER`; it does not
use privileged mode, host networking, a host PID namespace, or a
...
- Kubernetes package:
  `ghcr.io/pasturestack/kubernetes-package:v1.12.10-pasturestack.3`
  from [`PastureStack/kubernetes-package@9ce1ea2729782c6c63bb18a310ef1ba6fdfddf94`](https://github.com/PastureStack/kubernetes-package/tree/9ce1ea2729782c6c63bb18a310ef1ba6fdfddf94)
- etcd:
  `ghcr.io/pasturestack/etcd-compat:v2.3.7-pasturestack.1`
  from [`PastureStack/etcd-compat@107d43cf30ef67fce52879a739c53c3bc5c1c8ec`](https://github.com/PastureStack/etcd-compat/tree/107d43cf30ef67fce52879a739c53c3bc5c1c8ec)
- kubectl service:
  `ghcr.io/pasturestack/kubectl-service:v0.9.11-pasturestack.5`
  from [`PastureStack/kubectl-service@3da8f6116a17f49501370dcd1c9ae0a29c2be738`](https://github.com/PastureStack/kubectl-service/tree/3da8f6116a17f49501370dcd1c9ae0a29c2be738)
- host-file updater:
...
- Reviewed: 2026-08-07
- Vulnerability gate: the current kubectl-service release has 0 HIGH and 0
  CRITICAL findings. The current Kubernetes package retains its raw 0 CRITICAL
  and 2 HIGH report plus the source-linked OpenVEX review; the applicable result
  is 0 CRITICAL and 0 HIGH. The release assets preserve both results instead of
  hiding the scanner output.
- Distribution gate: every listed semantic tag passed anonymous GHCR manifest
  access without stored registry credentials

Catalog version 4 is the default and uses the current package and kubectl
service releases above. Catalog versions 2 and 3 remain byte-for-byte immutable
for definition comparison and rollback; their older image locks remain recorded
in `catalog-images.json` but are not the default.
...
The three Windows entries restore only the official upstream Windows
...
- Reviewed: 2026-07-26
- Source gate: all three services passed their source tests and Windows
  `amd64` cross-compilation; the resulting executables were inspected as
  64-bit Windows console PE files.
- Vulnerability gate: all three source release trees have 0 HIGH, 0 CRITICAL,
  and 0 detected secrets with Trivy 0.72.0.
- Distribution gate: every Windows semantic tag is public and passed anonymous
  manifest and configuration access.

The Windows images use the Windows Server 2022 (`ltsc2022`) container ABI and
are selected only by the `io.rancher.host.os=windows` compatibility scheduler
label. The container-networking entry retains one historical selector value
solely because existing Windows agents consume it as a protocol identifier;
it is documented in the template and is not displayed as product branding.
No Windows host was available for an end-to-end container, Metadata, DNS, ECR,
NAT, or transparent-networking lifecycle. All three entries therefore remain
release candidates and must not be represented as Windows-runtime accepted.
...
- Reviewed: 2026-07-26
- Vulnerability gate: 0 HIGH and 0 CRITICAL findings with Trivy 0.70.0
- Secret gate: 0 image-secret findings with Trivy 0.70.0
- Distribution gate: public GHCR and unauthenticated semantic-version-tag pull passed
- Component integration: link-local Metadata, long polling, HAProxy configuration and socket, credential compatibility, event reporting, TCP readiness, and stdout/stderr stream separation passed

This entry remains a release candidate. Version `v0.3.16` keeps the established
link-local default from `v0.3.15` and routes routine lifecycle messages to
standard output while reserving standard error for actual failures. A fresh
