#!/usr/bin/env python3
"""Fail closed unless every enabled catalog image is immutable and reviewed."""
...
def main() -> int:
    ...
    for item in data.get("images", ()):
        ...
        if ":latest@" in reference:
            blocker(f"mutable latest tag is prohibited: {reference}", failures)
        ...
        for path in compose_files:
            for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
                ...
                if not DIGEST_PATTERN.fullmatch(reference):
                    blocker(f"image is not pinned by digest at {relative}:{line_number}: {reference}", failures)
