...
# PastureStack IPsec Overlay 0.3.4
...
- Image: `ghcr.io/pasturestack/ipsec-vxlan-overlay-network:v0.14.31` is pinned to its published manifest digest in `catalog-images.json`. The GitHub Release itself is not immutable.
- Version `6` retains the version `5` port-8111 handoff and keeps the shared charon daemon running when one peer is temporarily offline; periodic health reconciliation retries the missing CHILD_SA. Earlier template versions remain available and are not rewritten.
...
The published `v0.14.31` image is locked by its real manifest digest. The isolated native nftables, iptables-nft, and iptables-legacy gates must remain green. Managed upgrade and a peer restart must verify that a temporarily offline peer does not tear down healthy associations, while the new router still waits for port 8111 without changing Network Plugin Manager's firewall ownership. Live results for this version must be recorded separately before declaring the full managed lifecycle passed.
