...
# PastureStack IPsec Overlay 0.3.5
...
- Image: `ghcr.io/pasturestack/ipsec-vxlan-overlay-network:v0.14.32` is pinned to its published manifest digest in `catalog-images.json`. The GitHub Release itself is not immutable.
- Version `7` retains the version `6` peer-retry and port-8111 handoff behavior. The new per-peer uniqueness policy replaces concurrent IKE SAs; a stuck `DELETING` SA is removed only after a healthy replacement CHILD_SA exists. Earlier template versions remain available and are not rewritten.
...
The published `v0.14.32` image is locked by its real manifest digest. The isolated native nftables, iptables-nft, and iptables-legacy gates must remain green. Managed upgrade and a peer restart must verify that a temporarily offline peer does not tear down healthy associations, that one peer converges to one working IKE SA, and that the new router still waits for port 8111 without changing Network Plugin Manager's firewall ownership. Live results for this version must be recorded separately before declaring the full managed lifecycle passed.
