...
Version 5 uses Network Plugin Manager `v0.8.17`. It retains the single-backend selection and unchanged Metadata Service and Internal DNS images from version 4. The manager rejects malformed per-host subnet labels before touching host firewall rules. For the optional per-host-subnet network, it preserves container source IPs across active peers and permits only peer-to-local-subnet forwarding. The image's release provenance and digest are recorded in `catalog-images.json`.
