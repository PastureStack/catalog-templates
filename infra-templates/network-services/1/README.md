...
All three images are public PastureStack GHCR packages referenced by semantic
version tags. Published version tags are never overwritten; release checksums
retain the verification evidence. Its integrated single-host gate passed with
Metadata Healthcheck and IPsec Overlay. Install and activate Metadata
Healthcheck before treating infrastructure health as authoritative, then
install this stack before IPsec Overlay. The stack remains a release candidate
until its complete multi-host managed-network lifecycle has passed on isolated
hosts.
