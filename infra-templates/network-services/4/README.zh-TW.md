...
此版本為網路外掛管理器新增單一、可選擇的主機防火牆後端；中繼資料服務與內部 DNS 沿用版本 3。候選範本指向已正式發布的網路外掛管理器 `v0.8.13`，其官方映像的 manifest digest、來源 revision 與成品掃描結果已記錄於 `catalog-images.json`。這份商店候選仍須完成對應後端的主機生命週期驗收，才可部署。
