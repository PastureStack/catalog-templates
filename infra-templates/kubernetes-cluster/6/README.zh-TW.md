...
本版將六個控制平面與節點服務更新為
`kubernetes-package:v1.12.10-pasturestack.4`，並將兩個商店操作服務更新
為 `kubectl-service:v0.9.11-pasturestack.7`。etcd 服務維持使用已審核的
`etcd-compat:v2.3.7-pasturestack.2`。商店範本第 5 版保持位元組完全
不變，供管理者比對定義或回復。

新版 Kubernetes 套件加入經審核的 Docker 29 與統一 cgroup v2 相容
修補。成品 CycloneDX SBOM 共 437 個元件；原始 Trivy 報告保留安裝的
`cryptography 48.0.1` 所對應的 Critical 0、High 2，公開 OpenVEX 則
記錄已驗證離線 Azure CLI 執行路徑的適用 Critical 0、High 0。原始
發現沒有被刪除或改標。kubectl 服務成品 CycloneDX SBOM 共 313 個
元件，掃描結果為 Critical 0、High 0、映像機密資料 0。

Docker 29 會拒絕以私有掛載傳播模式重新建立 Kubelet 服務。本版在兩個
Docker 資料根目錄綁定掛載明確使用 `rslave`，讓主機端掛載事件能單向
傳入 Kubelet 容器，同時不把容器內的掛載事件反向傳回主機。既有
Kubelet 狀態目錄仍保留原本的共享掛載傳播契約。

本版仍是 Kubernetes 1.12、etcd 2.3 與 Helm 2 的相容移轉邊界，不代表
這些 EOL 元件重新獲得支援，也不代表所有發行資料都已完成移轉。升級
前必須備份並驗證 etcd 與 Helm 發行資料，完成分階段 Helm 與 Kubernetes
移轉後，才可移除相容服務。
