...
This revision moves every control-plane service to
`kubernetes-package:v1.12.10-pasturestack.3` and both catalog-operation services
to `kubectl-service:v0.9.11-pasturestack.3`. The matching package release uses
the reviewed `tiller:v2.17.0-pasturestack.2` compatibility image and verifies
all Helm 2 release records before and after its rolling update. The previous
template remains immutable as version 2 so an operator can compare or roll
back the Catalog definition.
