...
- The Vault-volume candidate adds one non-root environment bridge. Its
  renewable issuing token is selected from an existing platform Secret and
  mounted read-only; it must never be placed in Compose environment values or a
  command line. The bridge enforces active-host RSA-PSS authentication, replay
  protection, a policy allowlist, response wrapping, and accessor revocation.
  The global driver uses the same bounded `tmpfs` boundary as the secret-volume
  candidate and does not request privileged mode, host networking, a host PID
  namespace, or a container-engine socket.
