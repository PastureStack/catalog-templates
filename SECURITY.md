...
- Every image must have a reviewed source, license, immutable digest, architecture, and vulnerability assessment before release.
...
- The IPsec overlay candidate is intentionally privileged, joins host PID and network namespaces, installs CNI executables, reads scoped agent credentials, and changes XFRM, route, firewall, and bridge state. Its digest and compatibility contract must be validated together with the network-services stack on isolated hosts.
